...
HAVING QTD = (
	SELECT MAX(Qtd)
	FROM (
		SELECT MONTHNAME(a2.dataRealizacaoExame) Mes, COUNT(*) QTD 
		FROM Agendamento a2 
		GROUP BY Mes
		) Quantidade
	);
...
SELECT e.especialidade , COUNT(*) QTD 
FROM Exame e
INNER JOIN Exames_Realizados er ON er.fk_codExame = e.codExame 
INNER JOIN Agendamento a ON a.dataSolicitacao = er.fk_dataSolicitacao
WHERE YEAR(a.dataRealizacaoExame) = :ano
GROUP BY e.codExame 
HAVING QTD = (
	SELECT MAX(QTD)
